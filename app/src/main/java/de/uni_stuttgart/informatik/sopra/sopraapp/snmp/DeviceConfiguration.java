...
public class DeviceConfiguration {
    ...
    public DeviceConfiguration(DeviceConfiguration deviceConfiguration) {
        ...
        localEngineId = deviceConfiguration.getLocalEngineId();
    ...
    /**
     * method to ensure there is only one local engine id per device
     * cloning with constructor is considered
     *
     * @return
     */
    public OctetString getLocalEngineId() {
        if (localEngineId == null) {
            Log.d(DeviceConfiguration.class.getName(), "creating new local engine id");
//            setLocalEngineId(CockpitStateManager.getInstance().getLocalEngineId());
     ...
     * @return
     */
    public int getVersionSpecificTimeoutOffset() {
        // FIXME refactor away to a constant
        return 5000;
    }
